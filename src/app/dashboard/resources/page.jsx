...
import { MonitorSpeaker, Plus, Edit2, Trash2, X } from "lucide-react";
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Manage shared office equipment like projectors and laptops.
          </p>
        </div>
        {(user?.role === "Admin" || user?.role === "Manager") && (
          <button
            onClick={() => {
              setModalMode("add");
              setCurrentResource({ status: "active", quantity: 1 });
              setShowModal(true);
            }}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-700">
                ...
                {(user?.role === "Admin" || user?.role === "Manager") && (
                  <th className="px-6 py-4 text-sm font-semibold text-gray-900 dark:text-white text-right">
                    Actions
                  </th>
                )}
              </tr>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
              {resources.map((res) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`text-sm font-medium ${res.availableCount === 0 ? "text-red-600 dark:text-red-400" : "text-emerald-600 dark:text-emerald-400"}`}
                    >
                      {res.availableCount}
                    </span>
                  </td>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md overflow-hidden border border-gray-200 dark:border-gray-700 animate-in slide-in-from-bottom-4">
            ...
            <form onSubmit={handleSaveResource} className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  value={currentResource.status || "active"}
                  onChange={(e) =>
                    setCurrentResource({
                      ...currentResource,
