...
export default function ApprovalsPage() {
  ...
  const pastApprovals = meetings.filter(
  ...
  const handleApproval = async (id, newStatus) => {
    try {
      ...
      setMeetings(
        meetings.map((m) => (m.id === id ? { ...m, status: newStatus } : m)),
      );
    } catch (e) {
      console.error(e);
    }
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          Review and manage pending meeting requests.
        </p>
      </div>
        ...
        {pendingApprovals.length === 0 ? (
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-8 text-center">
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">
              There are no pending meeting requests to approve.
            </p>
          </div>
        ) : (
          <div className="grid gap-4">
            {pendingApprovals.map((meeting) => {
              ...
              return (
                <div
                  key={meeting.id}
                  className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-5 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4"
                >
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-xs text-gray-500 dark:text-gray-400">
                        Requested{" "}
                        {format(parseISO(meeting.createdAt), "MMM d, h:mm a")}
                      </span>
                    </div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-4 text-sm text-gray-600 dark:text-gray-400">
                      ...
                      <span className="flex items-center gap-1.5">
                        <Clock className="w-4 h-4" /> {meeting.startTime} -{" "}
                        {meeting.endTime}
                      </span>
                      <span className="flex items-center gap-1.5">
            ...
            <div className="divide-y divide-gray-100 dark:divide-gray-700">
              {pastApprovals.slice(0, 5).map((meeting) => (
                ...
                >
                  ...
                  <span
                    className={`px-2.5 py-1 rounded-full text-xs font-medium border ${meeting.status === "approved" ? "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-400 dark:border-emerald-800" : "bg-red-50 text-red-700 border-red-200 dark:bg-red-900/30 dark:text-red-400 dark:border-red-800"}`}
                  >
