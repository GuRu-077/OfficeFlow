...
export default function CalendarPage() {
  ...
  const getMeetingsForDayAndHour = (day, hour) => {
    return meetings.filter((m) => {
      ...
      // 1. Admins/Managers see everything.
      // 2. Employees only see 'confirmed'/'approved' meetings, OR their own 'pending' ones.
      const isOwner = m.requestedBy === user?.id;
      const isStaff = user?.role === "Admin" || user?.role === "Manager";
      const isApproved = m.status === "confirmed" || m.status === "approved";
      
      if (!isStaff && !isApproved && !isOwner) return false;
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case "confirmed":
      case "approved":
        return "bg-indigo-100 text-indigo-800 border-indigo-200 dark:bg-indigo-900/40 dark:text-indigo-300 dark:border-indigo-800/50";
      case "pending":
        return "bg-orange-100 text-orange-800 border-orange-200 dark:bg-orange-900/40 dark:text-orange-300 dark:border-orange-800/50";
      default:
      ...
      <div className="flex-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="grid grid-cols-8 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
          ...
          {weekDays.map((day, i) => {
            ...
            return (
              <div
                key={i}
                className={`p-3 text-center border-r border-gray-200 dark:border-gray-700 last:border-0 ${isToDay ? "bg-indigo-50/50 dark:bg-indigo-900/10" : ""}`}
              >
                <div className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                  {format(day, "EEE")}
                </div>
                <div
                  className={`mt-1 text-lg font-bold ${isToDay ? "text-indigo-600 dark:text-indigo-400" : "text-gray-900 dark:text-white"}`}
                >
                  {format(day, "d")}
                </div>
              </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {hours.map((hour) => (
            <div
              key={hour}
              className="grid grid-cols-8 border-b border-gray-100 dark:border-gray-700/50 min-h-[100px]"
            >
              ...
              {weekDays.map((day, dIdx) => {
                ...
                return (
                  <div
                    key={dIdx}
                    className="p-1 border-r border-gray-100 dark:border-gray-700/50 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-700/20 transition-colors relative group"
                  >
                    {dayMeetings.map((m) => (
                      <div
                        key={m.id}
                      ...
                      >
                        <div className="font-semibold truncate">
                          {m.status === "pending" ? "[PENDING] " : ""}{m.title}
                        </div>
