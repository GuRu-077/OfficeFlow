...
export default function ReportsPage() {
  ...
  useEffect(() => {
    if (filter === 'all') {
      ...
      setMeetings(allMeetings.filter(m => isAfter(parseISO(m.date), cutoff) || isSameDay(parseISO(m.date), cutoff)));
    }
  }, [allMeetings, filter]);
  ...
  const exportCSV = async () => {
    ...
    const headers = ['Meeting ID', 'Title', 'Date', 'Start Time', 'End Time', 'Room ID', 'Status', 'Requested By'];
  ...
  // Status Distribution
  const statusData = [
    { name: 'Confirmed', value: meetings.filter(m => m.status === 'confirmed').length },
  ...
  const trendData = weekDays.map(day => {
    const count = meetings.filter(m => isSameDay(parseISO(m.date), day)).length;
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Insights into office utilization and booking trends.</p>
        </div>
          ...
          <div className="relative">
            ...
            {showFilterDropdown && (
              <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-100 dark:border-gray-700 z-10">
                <div className="py-1">
                  <button onClick={() => { setFilter('7days'); setShowFilterDropdown(false); }} className={`block w-full text-left px-4 py-2 text-sm ${filter === '7days' ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'}`}>Last 7 Days</button>
                  <button onClick={() => { setFilter('30days'); setShowFilterDropdown(false); }} className={`block w-full text-left px-4 py-2 text-sm ${filter === '30days' ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'}`}>Last 30 Days</button>
                  <button onClick={() => { setFilter('all'); setShowFilterDropdown(false); }} className={`block w-full text-left px-4 py-2 text-sm ${filter === 'all' ? 'bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'}`}>All Time</button>
                </div>
              </div>
            )}
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-100 dark:border-gray-700 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Hours Booked</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-100 dark:border-gray-700 shadow-sm">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-6">Meetings This Week</h2>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={trendData}>
                ...
                />
                <Line type="monotone" dataKey="meetings" stroke="#4F46E5" strokeWidth={3} dot={{ r: 4, strokeWidth: 2 }} activeDot={{ r: 6 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        {/* Room Utilization */}
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-100 dark:border-gray-700 shadow-sm">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-6">Top Rooms Utilized</h2>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-100 dark:border-gray-700 shadow-sm lg:col-span-2 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex-1 w-full">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">Booking Status Overview</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">Distribution of meeting requests by their current approval and confirmation status.</p>
