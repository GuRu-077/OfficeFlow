...
export default function DashboardPage() {
  ...
  const getDynamicChartData = () => {
    ...
    activeMeetings.forEach((m) => {
      // Create a date object treating the date string as local time to prevent timezone shift
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <KPICard
          ...
          icon={
            <DoorOpen className="w-6 h-6 text-indigo-600 dark:text-indigo-400" />
          }
        />
        ...
        <KPICard
          ...
          icon={
            <MonitorSpeaker className="w-6 h-6 text-emerald-600 dark:text-emerald-400" />
          }
        />
        ...
        <KPICard
          ...
          value={meetings
            .filter((m) => m.date === format(new Date(), "yyyy-MM-dd") && (m.status === "confirmed" || m.status === "approved"))
            .length.toString()}
          subtitle={`${meetings.filter((m) => m.date === format(new Date(), "yyyy-MM-dd") && m.status === "pending").length} pending approval`}
          icon={
        ...
        <KPICard
          ...
          subtitle={
            pendingApprovals.length > 0 ? "Requires action" : "All caught up"
          }
          icon={
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {upcomingMeetings.length > 0 ? (
                upcomingMeetings.map((meeting) => {
                  ...
                  return (
                    <div
                      key={meeting.id}
                      className="px-6 py-4 flex items-center hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors cursor-pointer"
                    >
                      ...
                      <div className="ml-4 flex-1">
                        ...
                        <div className="flex items-center text-xs text-gray-500 dark:text-gray-400 mt-1 gap-2">
                          <span className="flex items-center gap-1">
                            <Clock className="w-3 h-3" /> {meeting.startTime} -{" "}
                            {meeting.endTime}
                          </span>
                          <span>•</span>
                          <span className="flex items-center gap-1">
        ...
        <div className="space-y-6">
          {/* Conflict Alerts - Removed to prevent confusion since it was hardcoded */}
...
function KPICard({ title, value, subtitle, icon, trend, alert }) {
  return (
    <motion.div
    ...
    >
      <div className="flex justify-between items-start">
        ...
        <div
          className={`p-3 rounded-xl ${alert ? "bg-orange-100 dark:bg-orange-900/30" : "bg-gray-50 dark:bg-gray-700"}`}
        >
          {icon}
        </div>
      </div>
