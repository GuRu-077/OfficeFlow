...
export default function NewMeetingPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [r, res, u, m] = await Promise.all([
        db.getRooms(),
        db.getResources(),
        db.getUsers(),
        db.getMeetings(),
  ...
  // Check if a specific room is booked during the selected time
  const isRoomBooked = (targetRoomId) => {
    if (!date || !startTime || !endTime) return false;
    
    return allMeetings.some((m) => {
      if (m.roomId !== targetRoomId) return false;
  ...
  const getNextAvailableTime = (targetRoomId) => {
    if (!date) return null;
    const roomMeetings = allMeetings
      .filter(m => m.roomId === targetRoomId && m.date === date && m.status !== "cancelled" && m.status !== "rejected")
      .sort((a, b) => new Date(`${date}T${a.endTime}`) - new Date(`${date}T${b.endTime}`));
      
    if (roomMeetings.length === 0) return "Any time is free.";
    const lastMeeting = roomMeetings[roomMeetings.length - 1];
    return lastMeeting.endTime;
  };
  ...
  const checkConflicts = async () => {
    ...
    if (overlapping) {
      const nextFree = getNextAvailableTime(roomId);
      return `Room is already booked for "${overlapping.title}" (${overlapping.startTime} - ${overlapping.endTime}). Suggestion: Next available slot today is at ${nextFree}.`;
    }
    return null;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setConflict(error.message || "An error occurred while creating the meeting.");
      if (error.alternatives) {
        setSuggestedRooms(error.alternatives);
      }
    } finally {
      setSubmitting(false);
    }
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-1">
          Book a room and invite participants.
        </p>
      </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {suggestedRooms.map((alt) => (
                      <button
                        ...
                        className="px-3 py-1.5 bg-white dark:bg-gray-800 border border-red-200 dark:border-red-800 rounded-lg text-xs font-medium text-red-700 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors shadow-sm"
                      >
                        Switch to {alt.name}
                      </button>
                    ))}
                  ...
                  {rooms.map((room) => {
                    ...
                    return (
                      <option key={room.id} value={room.id} disabled={booked}>
                        {room.name} (Capacity: {room.capacity}) {booked ? " - BOOKED" : ""}
                      </option>
                    );
                  })}
                </select>
            ...
            <div>
              ...
              <div className="border border-gray-200 dark:border-gray-600 rounded-lg max-h-48 overflow-y-auto bg-gray-50 dark:bg-gray-700/50">
                {allUsers.map((u) => (
                  <label
                    key={u.id}
                    className="flex items-center px-4 py-3 hover:bg-gray-100 dark:hover:bg-gray-600 cursor-pointer border-b border-gray-200 dark:border-gray-600 last:border-0 transition-colors"
                  >
                    <input
                      type="checkbox"
                      ...
                      onChange={(e) => {
                        if (e.target.checked)
                          ...
                          setSelectedParticipants(
                            selectedParticipants.filter((id) => id !== u.id),
                          );
                      }}
                    />
                ...
                {resources.map((res) => (
                  ...
                  >
                    ...
                    <div className="ml-3 flex-1 flex justify-between items-center">
                      <div>
                        <p
                          className={`text-sm font-medium ${res.availableCount === 0 ? "text-gray-400" : "text-gray-900 dark:text-white"}`}
                        >
                          {res.name}
                        </p>
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                      ...
                      <span
                        className={`text-xs px-2 py-1 rounded-full ${res.availableCount > 0 ? "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400" : "bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400"}`}
                      >
                        {res.availableCount > 0
                          ? `${res.availableCount} available`
                          : "Out of stock"}
                      </span>
                    </div>
                  </label>
