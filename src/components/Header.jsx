...
export function Header() {
  ...
  const markAsRead = (id) => {
    setNotifications(
      notifications.map((n) => (n.id === id ? { ...n, readStatus: true } : n)),
    );
  };
              ...
              <div className="max-h-96 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-700">
                {notifications.length > 0 ? (
                  notifications.map((notification) => (
                    <div
                      key={notification.id}
                      className={`p-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors ${!notification.readStatus ? "bg-indigo-50/30 dark:bg-indigo-900/10" : ""}`}
                    >
                      <div className="flex justify-between items-start gap-3">
                        ...
                        <div className="flex-1">
                          <p
                            className={`text-sm ${!notification.readStatus ? "text-gray-900 dark:text-white font-medium" : "text-gray-600 dark:text-gray-300"}`}
                          >
            ...
            <div className="absolute right-0 top-12 mt-2 w-56 bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden z-50">
              ...
              <div className="p-2 space-y-1">
                <Link
                  ...
                  className="w-full flex items-center gap-3 px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700/50 rounded-lg transition-colors text-left"
                >
