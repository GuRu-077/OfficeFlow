...
export function Sidebar() {
  ...
  const links = [
    ...
    { href: '/dashboard/calendar', label: 'Calendar', icon: CalendarDays, roles: ['Admin', 'Manager', 'Employee'] },
    { href: '/dashboard/rooms', label: 'Rooms', icon: DoorOpen, roles: ['Admin', 'Manager'] },
    { href: '/dashboard/resources', label: 'Resources', icon: MonitorSpeaker, roles: ['Admin', 'Manager'] },
    { href: '/dashboard/approvals', label: 'Approvals', icon: CheckSquare, roles: ['Admin', 'Manager'] },
    ...
    <div className="w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col h-screen sticky top-0">
      <div className="h-16 flex items-center px-6 border-b border-gray-200 dark:border-gray-700">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xl font-bold text-gray-900 dark:text-white">OfficeFlow</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
        {visibleLinks.map((link) => {
          ...
          return (
            ...
            >
              <Icon className={cn("w-5 h-5", isActive ? "text-indigo-700 dark:text-indigo-400" : "text-gray-400 dark:text-gray-500")} />
              {link.label}
            </Link>
          );
        })}
